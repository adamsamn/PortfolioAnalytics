...
def rolling_pairwise_corr_fig(
    ...
    window, plotted alongside the benchmark's cumulative return.

    On weekly data, 26 weeks (~6 months) is a sensible default. 13 weeks is
    noisy, 52 weeks smooths out short-term spikes. The benchmark cumulative
    return is plotted on a secondary axis so the user can see how holding
    co-movement evolves through up- and down-cycles.
    """
    ...
    # Benchmark cumulative return on secondary axis, aligned to the same window
    if benchmark in returns.columns:
        bench_cum = (1 + returns[benchmark]).cumprod()
        bench_cum = bench_cum.reindex(mean_corr.index).dropna()
        ax_bench = ax_corr.twinx()
        ax_bench.plot(bench_cum.index, bench_cum.values, color='gray', alpha=0.7,
                      linewidth=1.4, label=f'{benchmark} cumulative return')
        ax_bench.set_ylabel(f'{benchmark} Cumulative Return', color='gray')
